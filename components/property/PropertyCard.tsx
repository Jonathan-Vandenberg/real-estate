...
import { Agent, ImageProduct, Image_Category, Property } from "../../types";
...
export default function PropertyCard({ property, image, agent }: IProps) {
  return (
    <>
                ...
                >
                  <div className="shadow p-4 rounded-lg bg-white">
                    <div className="flex justify-center relative rounded-lg overflow-hidden h-52">
                      <div className="transition-transform duration-500 transform ease-in-out hover:scale-110 w-full h-[20rem]">
                        {image.map((img) => {
                          if (
                            img.propertyId === listing.id &&
                        ...
                        <span className="absolute inset-0 bg-black opacity-10 hover:opacity-0 transition-opacity duration-1000" />
                      </div>
                      ...
                      {listing?.featured && (
                        <span className="absolute top-0 left-0 inline-flex mt-3 ml-3 px-3 py-2 rounded-lg z-10 bg-[rgba(67,79,248,0.83)] text-xs font-medium text-white select-none">
                          Featured
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-4">
                      <h2
                        className="font-medium text-base text-md md:text-lg line-clamp-1"
                        title={listing!.title}
                      >
                        {listing!.title}
                      </h2>
                        ...
                        <div>
                          <span
                            className={classNames(
                              "px-2 py-1 font-semibold rounded-full text-white",
                              listing.status === "SOLD"
                                ? "bg-primary-gradient"
                    ...
                    <div className="grid grid-cols-2 grid-rows-2 gap-4 mt-8">
                      <div className="inline-flex flex-col xl:flex-row xl:items-center">
                        ...
                        <span className="mt-2 xl:mt-0 text-sm">
                          {convertEnum(listing?.propertyCategory)}
                        </span>
                      </div>
                      <div className="inline-flex flex-col xl:flex-row xl:items-center text-black">
                        <Furnished className="text-black" />
                        <span className="mt-2 xl:mt-0 text-sm">
                          Partly furnished
                        </span>
                      </div>
                          ...
                          a.id === listing.agentId && (
                            <div key={i}>
                              <div className="flex items-center">
                                ...
                                <div className="ml-2 line-clamp-1 text-md font-light ">
                                  {a.firstName}
                                </div>
                              </div>
                            </div>
